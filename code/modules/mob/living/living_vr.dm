...
/mob/living/verb/set_metainfo()
	...
	if(usr != src)
		return
	var/new_metadata = (tgui_input_text(usr, "Enter any information you'd like others to see, such as Roleplay-preferences. This will not be saved permanently unless you click save in the OOC notes panel!", "Game Preference" , html_decode(ooc_notes), multiline = TRUE,  prevent_enter = TRUE))
...
/mob/living/proc/set_metainfo_likes(var/reopen = TRUE)
	if(usr != src)
		return
	var/new_metadata = (tgui_input_text(usr, "Enter any information you'd like others to see relating to your LIKED roleplay preferences. This will not be saved permanently unless you click save in the OOC notes panel! Type \"!clear\" to empty.", "Game Preference" , html_decode(ooc_notes_likes), multiline = TRUE,  prevent_enter = TRUE))
	if(new_metadata && CanUseTopic(usr))
...
/mob/living/proc/set_metainfo_dislikes(var/reopen = TRUE)
	if(usr != src)
		return
	var/new_metadata = (tgui_input_text(usr, "Enter any information you'd like others to see relating to your DISLIKED roleplay preferences. This will not be saved permanently unless you click save in the OOC notes panel! Type \"!clear\" to empty.", "Game Preference" , html_decode(ooc_notes_dislikes), multiline = TRUE,  prevent_enter = TRUE))
	if(new_metadata && CanUseTopic(usr))
		if(new_metadata == "!clear")
			new_metadata = ""
...
/mob/living/verb/set_custom_link()
	...
	if(usr != src)
		return
	var/new_link = (tgui_input_text(usr, "Enter a link to add on to your examine text! This should be a related image link/gallery, or things like your F-list. This is not the place for memes.", "Custom Link" , html_decode(custom_link), max_length = 100, encode = TRUE,  prevent_enter = TRUE))
