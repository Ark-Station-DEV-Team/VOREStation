...
/obj/item/clothing/accessory/collar/shock/Topic(href, href_list)
	...
	if(((istype(usr, /mob/living/carbon/human) && ((!( ticker ) || (ticker && ticker.mode != "monkey")) && usr.contents.Find(src))) || (usr.contents.Find(master) || (in_range(src, usr) && istype(loc, /turf)))))
		...
		if(href_list["tag"])
			var/str = copytext_char(reject_bad_text(tgui_input_text(usr,"Tag text?","Set tag","",MAX_NAME_LEN)),1,MAX_NAME_LEN)
			if(!str || !length(str))
...
/obj/item/clothing/accessory/collar/attack_self(mob/user as mob)
	...
	var/str = copytext_char(reject_bad_text(tgui_input_text(user,"Tag text?","Set tag","",MAX_NAME_LEN)),1,MAX_NAME_LEN)
